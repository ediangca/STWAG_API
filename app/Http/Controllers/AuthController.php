...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        try {
            $request->validate([
                ...
                'contactno' => 'required|string||max:11|unique:users',
    ...
    public function getDownlinesByID(Request $request, $id)
    {
        ...
        foreach ($downlines as $downline) {
            $downline->total_downlines = User::where('uplinecode', $downline->referencecode)->count();
        }

        return response()->json(['total' => count($downlines), 'downlines' => $downlines]);
    }
    ...
    public function getDownlinesByLevel($user_id)
    {
        ...
        // Log::info('Initial current level users', ['level' => $level, 'currentLevelUsersCount' => count($currentLevelUsers)]);
